                ...
                <main>
                    <div class="container-fluid">
                        ...
                        <div class="form-group row">
                            <label for="inputtahun" class="col-form-label pl-3">Tahun</label>
                            <div class="col-sm-2">
                            <select class="form-control" name="tahun" id="exampleFormControlSelect1">
                            <?php
                            $conn= open_db();
                            
                            $sql1 = $conn->query("SELECT DISTINCT tahun FROM simpanan");
                            while ($data=$sql1->fetch_assoc()){
                                $tahun = $data['tahun'];
                                // $bulan1 = $data['bulan'];
                                // $tahun = substr($bulan1, 0, 4);
                                $text = $tahun;
                                echo "<option value='$tahun'>$text</option>";
                            }
                            ?>
                            </select>
                        </div>
                        ...
                        <table id="table-user" class="table table-striped table-bordered table-sm" style="width:100%">
                            ...
                            <tbody>
                                        <?php
                                            ...
                                            error_reporting(0);
                                            $tahun = $_GET['tahun'];
                                            $sql = $conn->query("select tanggal, id, id_user, simpanan_pokok, simpanan_wajib, simpanan_sukarela, sum(simpanan_pokok + simpanan_wajib + simpanan_sukarela) AS count from simpanan where tahun='$tahun' AND status='disetujui' GROUP BY id");
                                            ...
                                            // menjumlahkan data kolom yang di (sum) di line 74 atau mensimpelkan nama
